...
import { Loader2, Signal, Smartphone } from "lucide-react";

interface AppsnCheckoutModalProps {
  reference: string;
  onClose: () => void;
  onSuccess: (reference: string) => void;
}

export function AppsnCheckoutModal({
  reference,
  onClose,
  onSuccess,
}: AppsnCheckoutModalProps) {
  ...
  async function handlePayment(outcome: "SUCCESS" | "FAILED") {
    ...
    if (outcome === "SUCCESS") {
      ...
      // Just close or allow retry?
      // Usually decline means failed transaction.
      // We'll keep the failed state visible until user closes.
    }
  }
  ...
  const ModalWrapper = ({ children }: { children: React.ReactNode }) => (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in duration-300 z-50">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden animate-in zoom-in-95 duration-300 relative">
        ...
        <button
          onClick={onClose}
          className="absolute top-2 right-2 text-slate-400 hover:text-slate-600 p-2 rounded-full hover:bg-slate-100 transition-colors z-10"
        >
          <svg
            className="w-5 h-5"
            fill="none"
