...
export default function OrganizerTicketingPage() {
  ...
  const fetchEvents = async () => {
    setLoading(true);
    try {
      const response = await fetch("/api/organizer/events?type=TICKETING");
      if (response.ok) {
        const data: Event[] = await response.json();
        setEvents(data);

        // Calculate stats
        const revenue = data.reduce((acc, ev) => acc + ev.totalRevenue, 0);
        const sold = data.reduce((acc, ev) => acc + ev.ticketsSold, 0);
        const active = data.filter((ev) => ev.status === "LIVE").length;
        setStats({
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-500">
            Manage your ticket sales and view performance.
          </p>
        </div>
        <button
      ...
      <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
        <div className="p-4 border-b border-slate-200 flex items-center justify-between">
          <h2 className="font-semibold text-slate-900">Ticketing Events</h2>
          <button
            onClick={fetchEvents}
              ...
              return (
                ...
                >
                  <div className="flex items-start gap-4">
                    <div className="w-16 h-16 bg-blue-100 rounded-lg flex items-center justify-center flex-shrink-0 text-blue-600">
